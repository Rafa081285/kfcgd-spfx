...
**Hereda de:** `Document`  
**Biblioteca objetivo (default):** `Gestor Documental` (parametrizable)
...
| 18 | Documento general relacionado | `GD_DocumentoGeneral` | URL (Hyperlink) | No | No | **Nuevo.** Enlace al Documento general (PO/IT) padre. |

> **Leyenda:**
> - *Reutilizado*: campo ya existente, creado por `provisioning/02-sitecolumns.ps1` o `provisioning/05-taxonomyfields.ps1`.
...
- **Tipo:** URL (Hyperlink) — el valor es la URL al ítem de SharePoint del documento general en la misma biblioteca o en otra URL.
- **Uso recomendado:** al crear un Documento relacionado, copiar la URL del Documento general padre y pegarla en este campo.
- **Alternativa futura:** si se requiere una relación más estructurada, considerar implementar un campo `Lookup` a la misma biblioteca (requiere adaptación del script).
...
- [ ] Term Set está marcado como *Closed*
